...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        names='Launch Site', 
        title='Success of all sites')
        return fig
    else:
        filtered_df = spacex_df[spacex_df['Launch Site'] == entered_site]
    
        # Assuming 'class' indicates success (1) or failure (0)
        success_counts = filtered_df['class'].value_counts().reset_index()
        success_counts.columns = ['Outcome', 'Count']
        success_counts['Outcome'] = success_counts['Outcome'].map({1: 'Success', 0: 'Failure'})
    
        fig = px.pie(success_counts, values='Count', names='Outcome', title=f'Launch Success at Site {entered_site}')
...
def get_scatter_plot(entered_site, payload_range):
    ...
    filtered_df2 = spacex_df[(spacex_df['Payload Mass (kg)'] >= min_payload) & (spacex_df['Payload Mass (kg)'] <= max_payload)]
